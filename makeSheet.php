...
function insertStudentAnswer($dbc, $workingTable, $suppliedAnswer, $time){
	
	$insertQuery = "INSERT INTO $workingTable(`studentAnswer`, `timeTaken`) VALUES ('$suppliedAnswer', '$time')";
...
function masterWorksheetOperations($dbc, $masterName, $tableName){
	...
	if(!empty($initialCheckArray)){
		...
		if(!$initialValueInsertRun) {
			echo "There was a problem creating the Master table for this student. Sorry.";
		}
	}
...
`studentAnswer` VARCHAR( 2000 ) NOT NULL ,
...
if (isset($_POST['submitted'])){
	...
	#insert  $suppliedAnswer to Workingtable:	
	
	insertStudentAnswer($dbc, $workingTable, $suppliedAnswer, $time);
...
<fieldset >
<legend><h1><?php echo substr($weekSheet, 3) ?></h1></legend><br />
